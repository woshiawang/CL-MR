...
from dataset_contrastive import get_dataset, AVSDDataSet, collate_fn
...
def get_data_loaders_new(args, tokenizer):
    ...
    train_dataset = AVSDDataSet(train_data[0], tokenizer, (train_data[1], valid_data[1]), drop_rate=1, train=True)
    valid_dataset = AVSDDataSet(valid_data[0], tokenizer, (valid_data[1], train_data[1]), drop_rate=1, train=False)
...
def train():
    ...
    def update(engine, batch):
        ...
        token_type_ids = torch.cat([torch.ones((i3d.size(0), i3d.size(1))).long().to(args.device) * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-2]), token_type_ids], dim=1)
